...
MeshData LoadMesh( const std::string& pFile, bool instanced)
{
   ...
   BufferIndexedVerts(mesh,instanced);
   mesh.mNumIndices = mesh.mScene->mMeshes[0]->mNumFaces*3;
	return mesh;
}
...
void BufferIndexedVerts(MeshData& meshdata,bool instanced)
{
	...
	tex_coord_loc = glGetAttribLocation(program, "tex_coord_attrib");
	normal_loc = glGetAttribLocation(program, "normal_attrib");
	transform_loc = glGetAttribLocation(program, "transform_attrib");
	...
	if (instanced) {
		...
		glm::vec3 transform_data[num_instances];
		...
		for (int i = 0; i < num_instances; i++) {
			float angleTemp = float(i) / float(num_instances)*3.1415926*2.0;
			transform_data[i] = glm::vec3(sin(angleTemp)+float(rand()%100)/100.0*s, float(rand() % 100) / 100.0*s/2.0, cos(angleTemp) + float(rand() % 100) / 100.0*s)*k;
			tmax = std::max(tmax, glm::length(transform_data[i]));
			tmin = std::min(tmin, glm::length(transform_data[i]));
		}
		float xmax = tmax;
		xmax = tmin;

		glGenBuffers(1, &meshdata.mVboTransform);
		glBindBuffer(GL_ARRAY_BUFFER, meshdata.mVboTransform);
		glBufferData(GL_ARRAY_BUFFER, num_instances * sizeof(glm::vec3), transform_data, GL_STATIC_DRAW);
		glVertexAttribPointer(transform_loc, 3, GL_FLOAT, GL_FALSE, 0, NULL);

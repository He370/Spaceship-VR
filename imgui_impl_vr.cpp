...
void SetCurrentContextToActiveTexture()
{
	if (g_ActiveTexture >= 0 && g_ActiveTexture < g_MaxTextures)
	{
...
void ImGui_Impl_VR_SetGuiPose(glm::mat4& P)
{
	g_Mgui = P;
}
...
{
	return ImVec2(g_TexWidth, g_TexHeight);
}
...
void ImGui_Impl_VR_TriggerHapticPulse(vr::TrackedDeviceIndex_t device, short int duration = 500)
{
	...
	g_HMD->TriggerHapticPulse(left_controller, axis, duration);
}
...
void ImGui_Impl_VR_GrabScreenshot()
{
	...
	std::string preview_name("C:/Users/Lindun He/Desktop/preview_ss/");
	std::string vr_name("C:/Users/Lindun He/Desktop/vr_ss/");

	preview_name += std::to_string(ix);
	vr_name += std::to_string(ix);
	vr::EVRScreenshotError err = g_pScreenShots->RequestScreenshot(&handle, vr::VRScreenshotType_Stereo, preview_name.c_str(), vr_name.c_str());

	//char filename[1024];
	//uint32_t len = g_pScreenShots->GetScreenshotPropertyFilename(handle, vr::EVRScreenshotPropertyFilenames::VRScreenshotPropertyFilenames_Preview, filename, 1024, &err);
	ix++;
}
...
static const char* ImGui_Impl_VR_GetClipboardText(void*)
{
	//return SDL_GetClipboardText(); //any openvr equivalent here?
	return 0;
}

static void ImGui_Impl_VR_SetClipboardText(void*, const char* text)
{

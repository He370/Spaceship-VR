...
public:
	...
	void RenderScene( vr::Hmd_Eye nEye, GLuint fbo);
...
bool CMainApplication::HandleInput()
{
	...
	while( m_pHMD->PollNextEvent( &event, sizeof( event ) ) )
	{
		...
		if(b_spaceship_mode==false) ImGui_Impl_VR_ProcessEvent(&event);
		vr_event_callback(&event);
	}
	...
	for( vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; unDevice++ )
	{
		...
		if( m_pHMD->GetControllerState( unDevice, &state ) )
		{
			...
			glm::mat4 pose = Matrix4ToGlm(m_rmat4DevicePose[unDevice]);
			if (b_spaceship_mode == false) ImGui_Impl_VR_ProcessController(unDevice, &state, Matrix4ToGlm(m_rmat4DevicePose[unDevice]));
			vr_motion_callback(unDevice, &state, pose);
		}
	}
	if (b_spaceship_mode == false) ImGui_Impl_VR_Poll3dMouse();
...
void CMainApplication::RunMainLoop()
{
	...
	//InitRaycast();
	vr_init(m_pHMD);
	initOpenGl(m_nRenderWidth, m_nRenderHeight);
...
void CMainApplication::RenderStereoTargets()
{
	...
	glBindFramebuffer( GL_FRAMEBUFFER, leftEyeDesc.m_nRenderFramebufferId );
 	...
 	RenderScene( vr::Eye_Left , leftEyeDesc.m_nRenderFramebufferId);
 	glBindFramebuffer( GL_FRAMEBUFFER, 0 );
	...
	glBindFramebuffer( GL_FRAMEBUFFER, rightEyeDesc.m_nRenderFramebufferId );
 	glViewport(0, 0, m_nRenderWidth, m_nRenderHeight );
 	RenderScene( vr::Eye_Right, rightEyeDesc.m_nRenderFramebufferId);
 	glBindFramebuffer( GL_FRAMEBUFFER, 0 );
...
// Purpose:
//-----------------------------------------------------------------------------
void CMainApplication::RenderScene( vr::Hmd_Eye nEye, GLuint fbo)
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	...
	if( nEye == vr::Eye_Left )
	{
		...
		//DrawRaycast(P, V, M);
		display(P, V, M,fbo);
		if(!b_spaceship_mode) Draw3dGui(P, V, M);
	}
	...
	if( !bIsInputCapturedByAnotherProcess && !b_spaceship_mode)
	{
	...
	for( uint32_t unTrackedDevice = 0; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; unTrackedDevice++ )
	{
		...
		if(!b_spaceship_mode) m_rTrackedDeviceToRenderModel[ unTrackedDevice ]->Draw();
	}
